...
function formatTime(timestamp: string): string {
  ...
  return isNaN(t.getTime()) ? timestamp : t.toLocaleTimeString();
}

export default function AlertsPanel() {
  const { alerts, connected } = useFirebasePredictions();
          ...
          <tbody className="divide-y divide-slate-700/30">
            {alerts.length === 0 ? (
              ...
              alerts.map((alert, index) => (
                <tr key={`${alert.node}-${alert.timestamp}-${index}`} className="hover:bg-slate-800/20 transition-colors">
